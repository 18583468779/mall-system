<template>
  <div>
    ...
    <div class="flex items-center justify-end">
      <!-- 添加购物车按钮 -->
      <el-button v-if="bookItem?.purcharsenum === 0" type="primary"
      ...
      <div v-else class="flex items-center gap-3">
        <!-- 减少数量 -->
        <el-button v-show="bookItem.purcharsenum > 1" type="primary" circle size="small" class="!w-6 !h-6"
          @click.stop="addOrSubtrBookToShopCart(bookItem, 'sub', $event)">
          <span class="text-lg leading-none">-</span>
        </el-button>

        <!-- 删除按钮 -->
        <el-button v-show="bookItem.purcharsenum === 1" type="danger" circle size="small" class="!w-6 !h-6"
        ...
        <!-- 当前数量 -->
        <span v-if="bookItem.purcharsenum !== 1" class="min-w-[30px] text-center font-medium text-gray-700">
          {{ bookItem.purcharsenum }}
        </span>
        <el-button v-else-if="!isShowBtn" type="primary"
          class="!px-4 !py-4 !bg-red-100 hover:!bg-red-200 !text-red-600 !rounded-lg transition-colors border-none">
          <el-icon :size="16" class="mr-1">
            <ShoppingCartFull />
          </el-icon>
        ...
        <!-- 增加数量 -->
        <el-button v-if="false" type="primary" circle size="small" class="!w-6 !h-6"
