<template>
  <div class="rounded-lg p-4">
    <div class="flex flex-wrap gap-1.5 mb-4">
      <div
        v-for="item in firstCtgyComptuted"
        :key="item.firstctgyId"
        @click="handleFirstCtgyHover(item.firstctgyId)"
        class="px-3 py-1.5 rounded-md cursor-pointer transition-colors flex items-center text-sm"
        :class="{
          'bg-blue-100 text-blue-700': activeFirstIndex === item.firstctgyId,
          'hover:bg-gray-100 text-gray-600':
            activeFirstIndex !== item.firstctgyId,
        }"
      >
    ...
    <div class="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-3">
      <div
        ...
        class="border-l-2 border-blue-200 pl-2"
      >
        <!-- 二级分类标题 -->
...
const { storeRefs, getFirstCtgys, getSecondCtgys, changeTab, toBookInfo } =
  FstToThrdCtgy;
const { secondCtgyList, firstCtgyList, getFirstCtgy } = storeRefs;
